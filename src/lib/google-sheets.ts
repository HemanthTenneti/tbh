...
function getClient() {
  if (!SERVICE_EMAIL || !PRIVATE_KEY || !SHEET_ID) return null;
...
export async function appendRow(data: {
  ...
  const row: SheetRow = [timestamp, data.name, data.phone, data.instagram || ""];
  ...
  try {
    ...
    if (response.status === 200) {
      console.log(`[Google Sheets] Row appended: ${response.data.updates?.updatedRange}`);
      return { success: true };
    }
